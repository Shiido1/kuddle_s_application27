...
class AccommodationModalFilterFrame extends StatefulWidget {
  AccommodationModalFilterFrame(
      {Key? key, required ScrollController scrollController})
      : super(key: key);
  ScrollController? scrollController;

  @override
  State<AccommodationModalFilterFrame> createState() =>
      _AccommodationModalFilterFrameState();
}

class _AccommodationModalFilterFrameState
    extends State<AccommodationModalFilterFrame> {
  ScrollController? _scrollController;
  String pickText = '';
  List<String> isTapped = ['Hotel'];
  String _query = '';
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                serviceContainer(
                  'Hotel',
                  isTapped.contains('Hotel')
                      ? Colors.white
                      : theme.colorScheme.primary,
                  isTapped.contains('Hotel')
                      ? theme.colorScheme.primary
                     
                      : theme.colorScheme.primary.withOpacity(.05),'Hotel'
                ),
                SizedBox(
                  width: 14,
                ),
                serviceContainer(
                  'Villa',
                  isTapped.contains('Villa')
                      ? Colors.white
                      : theme.colorScheme.primary,
                  isTapped.contains('Villa')
                      ? theme.colorScheme.primary
                     
                      : theme.colorScheme.primary.withOpacity(.05),'Villa'
                ),
                SizedBox(
                  width: 14,
                ),
                serviceContainer(
                  'Apartment',
                  isTapped.contains('Apartment')
                      ? Colors.white
                      : theme.colorScheme.primary,
                  isTapped.contains('Apartment')
                      ? theme.colorScheme.primary
                      
                      : theme.colorScheme.primary.withOpacity(.05),'Apartment'
                ),
                SizedBox(
                  width: 14,
                ),
                serviceContainer(
                  'Guest House',
                  isTapped.contains('Guest House')
                      ? Colors.white
                      : theme.colorScheme.primary,
                  isTapped.contains('Guest House')
                      ? theme.colorScheme.primary
                     
                      : theme.colorScheme.primary.withOpacity(.05),'Guest'
                ),
                SizedBox(
                  width: 14,
                ),
                serviceContainer(
                  'Hostel',
                  isTapped.contains('Hostel')
                      ? Colors.white
                      : theme.colorScheme.primary,
                  isTapped.contains('Hostel')
                      ? theme.colorScheme.primary
                      
                      : theme.colorScheme.primary.withOpacity(.05),'Hostel'
                ),
                // SizedBox(
                //   width: 14,
                // ),
